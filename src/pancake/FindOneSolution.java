...
public class FindOneSolution {
    ...
    public static void main(String[] args) {
        ...
        if (rank == MASTER) {
            FindOneSolution.runMaster();
        } else {
            boolean ret = false;
            while (!ret) {
                ret = FindOneSolution.runSlave(rank);
            }
        }
        MPI.Finalize();
    }
    ...
    private static void runMaster() {
        ...
        main_loop: while (true) {
            ...
            Status status = MPI.COMM_WORLD.Recv(buf, 0, 1, MPI.OBJECT, MPI.ANY_SOURCE, FindOneSolution.I_NEED_WORK);
            ...
            MPI.COMM_WORLD.Isend(new Object[]{new HereIsWorkPackage(input, initialWork, maxDepth, -1)}, 0, 1, MPI.OBJECT,
                    status.source, FindOneSolution.HERE_IS_WORK);
            ...
            networkLoop: while (true) {
                ...
                switch (status.tag) {
                    case FindOneSolution.I_NEED_WORK:
                        ...
                        MPI.COMM_WORLD.Isend(new Integer[]{new Integer(status.source)}, 0, 1, MPI.OBJECT, rank,
                                FindOneSolution.GIVE_WORK);
                        break;
                    case FindOneSolution.NO_WORK_LEFT:
                        ...
                        break;
                    case FindOneSolution.HERE_IS_WORK:
                        ...
                        MPI.COMM_WORLD.Isend(buf, 0, 1, MPI.OBJECT, hereIsWorkPackage.requestingNode, FindOneSolution.HERE_IS_WORK);
                        nodesWithWork.add(hereIsWorkPackage.requestingNode);
                        break;
                    case FindOneSolution.I_HAVE_FOUND_A_SOLUTION:
                        ...
                        for (int receiver = 1; receiver < size; receiver++) {
                            MPI.COMM_WORLD.Isend(new Object[]{new EmptyPackage()}, 0, 1, MPI.OBJECT,
                                    receiver, FindOneSolution.SOLUTION_WAS_FOUND);
                        }
        ...
        while (true) {
            ...
            while (status != null) {
                ...
                switch (status.tag) {
                    case FindOneSolution.HERE_IS_WORK:
                        ...
                        break;
                    case FindOneSolution.GIVE_WORK:
                        if (stack.isEmpty()) {
                            MPI.COMM_WORLD.Send(new Object[]{new EmptyPackage()}, 0, 1, MPI.OBJECT, FindOneSolution.MASTER, FindOneSolution.NO_WORK_LEFT);
                        } else {
                            ...
                            MPI.COMM_WORLD.Send(new Object[]{hereIsWorkPackage}, 0, 1, MPI.OBJECT, FindOneSolution.MASTER, FindOneSolution.HERE_IS_WORK);
                        }
                        break;
                    case FindOneSolution.RESTART:
                        return false;
                    case FindOneSolution.SOLUTION_WAS_FOUND:
                        return true;
                    default:
            ...
            if (stack.empty()) {
                if (!workRequestSent) {
                    ...
                    MPI.COMM_WORLD.Send(new Object[]{new EmptyPackage()}, 0, 1, MPI.OBJECT, FindOneSolution.MASTER, FindOneSolution.I_NEED_WORK);
                    workRequestSent = true;
                }
            } else {
                ArrayList<Integer> solution = PancakeNetwork.search(input, stack, maxDepth);
                if (solution != null) {
                    MPI.COMM_WORLD.Send(new Object[]{new SolutionPackage(solution)}, 0, 1, MPI.OBJECT, FindOneSolution.MASTER,
                            FindOneSolution.I_HAVE_FOUND_A_SOLUTION);
                    return true;
                }
            }
        }
